...
3.系统日志区字体调大
4.系统日志区进度条始终最下
5.选择内存管理模型界面添加每种模型的介绍
6.输入框绑定事件，初次点击清空
7.菜单添加‘重新开始’按钮
8.页面大小设置项添加‘内存可以分为xx块’
9.分页模型中，如果页面过多，画布中数字重叠
10.分页模型中，如果页面过多，页表显示不完全
11.段页式中，如果页面过多，画布中数字重叠
12.段页式中，如果页面过多，段页表显示不完全
'''
from tkinter import *
...
def selectseg():
    ...
    dh = StringVar()
    duanhao = Entry(table, textvariable=dh)
    dh.set('输入段号：')
    duanhao.place(x=300, y=130)

    dpy = StringVar()
    duanpianyi = Entry(table, textvariable=dpy)
    dpy.set('输入段内地址：')
    ...
    dz = StringVar()
    dizhi = Entry(table, textvariable=dz, width=30)
...
def selectpage():
    ...
    e = StringVar()
    logicaddr = Entry(table, textvariable=e)
    e.set('此处输入逻辑地址：')
    ...
    yh = StringVar()
    yehao = Entry(table, textvariable=yh, width=30)
    yehao.config(state='readonly')
    yh.set('此处显示页号：')
    yehao.place(x=500, y=90)

    py = StringVar()
    yepianyi = Entry(table, textvariable=py, width=30)
    yepianyi.config(state='readonly')
    py.set('此处显示页内偏移：')
    yepianyi.place(x=500, y=130)

    kh = StringVar()
    kuaihao = Entry(table, textvariable=kh, width=30)
    kuaihao.config(state='readonly')
    kh.set('此处显示块号：')
    kuaihao.place(x=500, y=170)

    p = StringVar()
    physicaladdress = Entry(table, textvariable=p, width=30)
...
# 日志面板
logPanel = Frame(rootWindow, bd=3, relief='sunken')
Label(logPanel, text='系统日志区域').pack(side=TOP, anchor=W)

srl = Scrollbar(logPanel, orient=VERTICAL)
log = Text(logPanel, bg='#F0F0F0', width=117, height=21, relief='flat', state=DISABLED)
